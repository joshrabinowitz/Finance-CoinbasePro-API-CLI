...
my @allowed_actions = qw( top buy sell products accounts orders quotes trades fills cancel cancelall );
my $sleep = 1;
my $top_max = 1;
...
sub main {
    ...
    if ($action eq "accounts") {
        ...
        if ($gaccount->error) { die "$prog: There was an error " . $gaccount->error; }
        my @jaccounts = map { Finance::CoinbasePro::API::CLI::Account->new( $_ ) } @$account_list;
    ...
    # ORDERS
        ...
        for my $o (@open_orders) {
            printf( "%s: %s Action: %s %0.3f @ \$%4.2f (value \$%4.2f) [order id: %s]\n", 
                $prog, $o->{product_id}, $o->{side}, $o->{size}, $o->{price}, $o->{size}*$o->{price}, $o->{id} );
        }
        #print "$prog: $action: " . join(" ", map { $_->{id} } @$positions ) . "\n";
    }   
    # CANCEL and CANCEL-ALL
    elsif ($action eq "cancel") {
        die "$prog: cancel action needs id like: 74d59372-9719-4b36-a641-217d7c9e7fb0\n" unless length($id);
        if (ask("Are you sure you want to cancel order id $id? (Yes/No)", '[Yy][Ee][Ss]')) {
        ...
        my $order = Finance::GDAX::API::Order->new( @creds );
        if (ask("Are you sure you want to cancel all orders for all products?", '[Yy][Ee][Ss]')) {
    ...
    # TRADES
        ...
        for my $a_product (@products) {
            ...
            my @jtrades = map { Finance::CoinbasePro::API::CLI::Trade->new( $_) } @$trades;
            for my $j (@jtrades) {
    ...
    # BUY / SELL
        ...
        print "$prog: $action: initiating limit $action of $size $product at $price\n";
        my $order = Finance::GDAX::API::Order->new( @creds );
        ...
        $order->price( $price );
        $order->size( $size );
        if (ask("Are you sure you want to $action $size of $product at $price? (Yes/No)", '[Yy][Ee][Ss]')) {
            my $response = $order->initiate;
            if ($order->error) { die "Error: ".$order->error }
            else { print "$prog: $action: response is " . ddump($response) . "\n"; }
        } else {
...
sub top {
    ...
    while(!$top_max || $top_counter++ < $top_max) {
        for my $a_product( @products) {
            ...
            my ($from_currency, $to_currency) = get_product_currencies( $a_product );
            ...
            my $from_trading_offset = Finance::CoinbasePro::API::CLI::Value->new( currency=>$from_currency, num=>0 );
            my $to_trading_offset = Finance::CoinbasePro::API::CLI::Value->new( currency=>$to_currency, num=>0 );
            my $num_fills = scalar(@$fills)-1;
            my $show_max = min(9, $num_fills);
            FILL_LOOP:
            for my $i (0 .. $show_max) {
                my $fill = Finance::CoinbasePro::API::CLI::Fill->new( $fills->[$i] );
                printf( "%s: fill: %2d: %s\n", $prog, $counter++, $fill->to_str );
                $from_trading_offset->add( $fill->source_offset() );
                $to_trading_offset->add( $fill->destination_offset() );
            }
            printf( "%s: trading offset: %s, %s\n", $prog, $to_trading_offset->to_str(), $from_trading_offset->to_str() );

            # SHOW HOLDINGS
            my $holdings_value = Finance::CoinbasePro::API::CLI::Value->new( currency=>$from_currency, num=>0 );
            for my $account (@$account_list) {
                my $a = Finance::CoinbasePro::API::CLI::Account->new( $account );
                if ($a->{balance} > 0) { printf( "%s: Current Holding: %s\n" ,  $prog, $a->to_str() ); }
            }
        
            # SHOW PRICES
            printf( "%s: Ticker  : $a_product: NOW: %s: (bid %s, ask %s), VOLUME % 7d\n", 
                $prog, format_usd($ticker->{price}), format_usd($ticker->{bid}), format_usd($ticker->{ask}), $ticker->{volume} );
        }
